...
const fetchMessages = asyncHandler(async (req, res) => {
  ...
  let messages = await MessageModel.find({ chat: chatId })
    ...
    .sort({ createdAt: "desc" });
  let chat_ = await ChatModel.findById({ _id: chatId });
 // console.loglog({ chat_ })
  messages = messages.map((message) => {
    message.content = decryptMessage(message.content, chat_.keytwo, chat_.keyOne);
    return message;
  })


  //// console.loglog({ users, chat_ })
...
const sendMessage = asyncHandler(async (req, res) => {
 ...
 // console.loglog({ Mychatid: chatId })
  ...
  if (!attachment) {
   ...
   // console.loglog({ attachment })
    const directory = './backend/messageFiles/';
    ...
    let imgpath = Date.now() + "-" + attachment.originalname
    const filePath = directory + imgpath;

    // Write the buffer to the file
    fs.writeFile(filePath, attachment.buffer, (err) => {
      if (err) {
  ...
  let encryptedString = "";
  if (content) {
    encryptedString = encryptMessage(content, chat_.keyOne);
  }
  ...
  const populatedMsgPromise = MessageModel.findById(createdMessage._id)
    ...
    .populate({
      path: "chat",
      model: "Chat",
      select: "-groupAdmins -cloudinary_id -keyOne -keytwo",
    });
  ...
  populatedMessage.content = decryptMessage(populatedMessage.content, chat_.keytwo, chat_.keyOne);

 // console.loglog({ populatedMessage })
